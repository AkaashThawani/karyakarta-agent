"""
Pure LLM-Based Task Decomposer

Creates atomic, verifiable micro-steps using ONLY LLM reasoning.
No hardcoded patterns, fallbacks, or site-specific knowledge.
Everything is dynamic or fails cleanly.
"""
...
class TaskDecomposer:
    """
    Pure LLM-driven task decomposer with zero hardcoded logic.

    Key Features:
    - 100% LLM-based decomposition (no fallbacks)
    - Atomic micro-steps with validation
    - Dynamic learning from tool registry
    - Clean failure (no keyword matching)
    """

    def __init__(self, llm_service: Any):
        """
        Initialize pure LLM-based task decomposer.

        Args:
            llm_service: LLM service for decomposition (required, no fallbacks)
        """
    ...
    def decompose(self, task_description: str, task_id: str, context: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        if task_structure.get("steps"):
            ...
            print(
                f"[DECOMPOSER] ⚡ Using pre-analyzed steps (type: {structure_type}, no LLM call)")

            # DEBUG LOGGING
            print(f"[DECOMPOSER] 🔍 DEBUG: task_structure = {task_structure}")
            print(
                f"[DECOMPOSER] 🔍 DEBUG: steps = {task_structure.get('steps')}")
            print(
                f"[DECOMPOSER] 🔍 DEBUG: steps type = {type(task_structure.get('steps'))}")
            print(
                f"[DECOMPOSER] 🔍 DEBUG: steps length = {len(task_structure.get('steps', []))}")
        ...
        if not required_fields:
            ...
            if required_fields:
                print(
                    f"[DECOMPOSER] Detected required fields: {required_fields}")

        if not self.llm_service:
            print("[DECOMPOSER] No LLM service available - failing cleanly")
            return []

        try:
            subtasks = self._llm_decomposition(
                task_description, task_id, query_params, task_type)

            if subtasks:
                print(f"[DECOMPOSER] LLM generated {len(subtasks)} subtasks")

                # Add completeness check after extraction subtasks
                if required_fields:
                    subtasks = self._add_completeness_checks(
                        subtasks, required_fields, task_id)

                # Apply query params to subtasks if api_call tool is used
                if query_params:
                    subtasks = self._apply_query_params(subtasks, query_params)

                return subtasks
            else:
                print("[DECOMPOSER] LLM returned empty - failing cleanly")
                return []

        except Exception as e:
            print(
                f"[DECOMPOSER] LLM decomposition failed: {e} - failing cleanly")
            return []
    ...
    def _llm_decomposition(self, task_description: str, task_id: str, query_params: Dict[str, Any], task_type: str) -> List[Dict[str, Any]]:
        ...
        for i, subtask in enumerate(subtasks):
            if self._validate_subtask(subtask):
                ...
                subtask["description"] = subtask.get(
...
**Example Output:**
[{{"tool":"google_search","action":"query","parameters":{{"query":"cats"}},"description":"Search for cats"}},{{"tool":"playwright_execute",
    "action":"goto","parameters":{{"url":"https://example.com","method":"goto","args":{{}}}},"description":"Navigate to example.com"}}]
...
Return ONLY the JSON array (no markdown, no explanation):"""
    ...
    def _validate_subtask(self, subtask: Dict[str, Any]) -> bool:
        ...
        if not all(field in subtask for field in required_fields):
            print(
                f"[DECOMPOSER] Invalid subtask - missing required fields: {subtask}")
            return False
        ...
        if subtask["tool"] == "playwright_execute":
            ...
            if not has_method:
                print(
                    f"[DECOMPOSER] Invalid playwright subtask - missing method: {subtask}")
                return False
            ...
            if "method" in subtask and "method" not in subtask["parameters"]:
                ...
                print(
                    f"[DECOMPOSER] Moved method to parameters: {subtask['parameters']['method']}")

        return True

    def _parse_json_values(self, obj: Any) -> Any:
        """
        Recursively parse JSON strings in dict/list values.
        Handles cases where LLM returns JSON strings instead of objects.

        Args:
            obj: Object to parse (dict, list, or primitive)

        Returns:
            Object with JSON strings parsed to objects
        """
            ...
            for k, v in obj.items():
                if isinstance(v, str):
                    ...
                    try:
                        ...
                        print(f"[DECOMPOSER] ✓ Parsed JSON string for '{k}': {v} → {parsed[k]}")
                    except (json.JSONDecodeError, TypeError):
                        # Not JSON, keep as string
                        parsed[k] = v
                else:
                    # Recursively parse nested structures
                    parsed[k] = self._parse_json_values(v)
            return parsed
        elif isinstance(obj, list):
            return [self._parse_json_values(item) for item in obj]
        else:
            return obj
    ...
    def _create_sequential_subtasks(
        ...
        """
        print(
            f"[DECOMPOSER] Creating {len(steps)} sequential subtasks with tool assignments")
        ...
        for i, step in enumerate(steps):
            ...
            if isinstance(step, dict):
                ...
                llm_params_raw = step.get("parameters", {})

                # PARSE JSON STRING TO DICT if it's a string (handles LLM returning JSON strings)
                if isinstance(llm_params_raw, str):
                    try:
                        llm_params = json.loads(llm_params_raw)
                        print(f"[DECOMPOSER] ✓ Parsed JSON parameters: {llm_params}")
                    except json.JSONDecodeError as e:
                        print(f"[DECOMPOSER] ✗ Failed to parse parameters JSON: {llm_params_raw} - Error: {e}")
                        llm_params = {}
                else:
                    llm_params = llm_params_raw

                # Parse JSON strings within parameter values (handles LLM returning args as JSON strings)
                llm_params = self._parse_json_values(llm_params)
